...
3 13,16,19,22,1,4,7,10 * * * \
  gbrain jobs submit shell \
    --params '{"cmd":"node /path/to/your-openclaw/workspace/scripts/x-garrytan-daily.mjs","cwd":"/path/to/your-openclaw/workspace"}' \
...
```bash
gbrain jobs submit shell \
  --params '{"cmd":"node scripts/yc-sync.mjs","cwd":"/path/to/your-openclaw/workspace","env":{"YC_API_TOKEN":"'"$YC_API_TOKEN"'"}}'
```
