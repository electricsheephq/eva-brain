...
`--fix` writes a centralized backup under `~/.gbrain/backups/frontmatter/` for
every modified file. SLUG_MISMATCH errors are surfaced for manual review (not
auto-fixed) — gbrain derives slugs from path, so a mismatched slug usually means
the user renamed the file intentionally or the slug field is stale. Missing
frontmatter is optional metadata coverage for broad document sources; do not add
blanket `type: note` just to silence a check.
