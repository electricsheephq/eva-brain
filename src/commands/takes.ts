...
async function cmdResolve(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!slug || !rowNumStr || !outcomeStr) {
    console.error('Usage: gbrain takes resolve <slug> --row N --outcome true|false [--source-id <id>] [--value N --unit usd|pct|count] [--source "..."] [--by <slug>]');
    process.exit(1);
  }
  const rowNum = parseInt(rowNumStr, 10);
...
Subcommands:
  ...
  takes resolve <slug> --row N --outcome true|false [--source-id id] [--value N --unit usd|pct|count] [--source "..."] [--by <slug>]
