...
//
// Upstream v0.36 added a strong ZeroEntropy path plus `gbrain ze-switch`;
// Eva keeps that as an explicit opt-in upgrade lever, but fresh Eva installs
// and OpenClaw fleet updates size the primary text column for Voyage because
// that is the supported fleet posture and matches INSTALL_FOR_AGENTS.md.
...
export function isAvailable(touchpoint: TouchpointKind, modelOverride?: string): boolean {
  ...
  try {
    ...
    // OpenAI-compatible templates with empty model lists (LiteLLM,
    // llama-server, user-provided local servers) mean "operator must supply a
    // model", not "provider unavailable". resolveRecipe(modelStr) already
    // proved a concrete provider:model was configured.
