...
/**
 * Extend the resolved search-mode hash with the caller's federated source set.
 *
 * query_cache has a scalar `source_id` column for legacy/default isolation.
 * Federated OAuth reads can intentionally search several sources while still
 * carrying a primary `sourceId` such as `default`; if the source set is not
 * part of the cache identity, a federated result can be replayed to a later
 * scalar/default-only caller. Keep the schema stable and encode the canonical
 * source set into `knobs_hash`.
 */
export function cacheKnobsHashForSourceScope(knobsHash: string, sourceIds?: readonly string[]): string {
  if (!sourceIds || sourceIds.length === 0) return knobsHash;
  const canonical = Array.from(new Set(sourceIds.filter(Boolean))).sort().join(',');
  if (!canonical) return knobsHash;
  const h = createHash('sha256');
  h.update(canonical);
  return `${knobsHash}::sources:${h.digest('hex').slice(0, 16)}`;
}
