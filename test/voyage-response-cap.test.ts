...
describe('v0.31.8 — voyage Content-Length pre-check + per-item cap', () => {
  ...
  test('Layer 1 throws on Content-Length over the cap (not silent return)', async () => {
    const source = await Bun.file(new URL('../src/core/ai/gateway.ts', import.meta.url)).text();
    // The cap check must throw an error so the embed flow's
    // retry/backoff sees a failure, NOT a silent pass-through.
    expect(source).toMatch(/exceeds[^`]*MAX_VOYAGE_RESPONSE_BYTES[^`]*bytes/);
    expect(source).toMatch(/throw new VoyageResponseSizeError\([\s\S]{0,200}Voyage response Content-Length/);
  });
  ...
  test('Layer 2: per-embedding base64 cap fires inside the json.data iteration', async () => {
    ...
    expect(source).toMatch(/throw new VoyageResponseSizeError\([\s\S]{0,200}Voyage embedding base64/);
  });

  test('size-cap errors are rethrown instead of swallowed by JSON-transform fallback', async () => {
    const source = await Bun.file(new URL('../src/core/ai/gateway.ts', import.meta.url)).text();
    expect(source).toMatch(/catch \(err\) \{\s*if \(err instanceof VoyageResponseSizeError\) throw err;/);
  });
